...
class TaskRepository(val context: Context) : BaseRepository(context) {
    ...
    fun updateStatus(id: Int, comlete: Boolean, listener: APIListener<Boolean>) {
        if (!isConnectionAvailable(context)) {
            listener.onFailure(context.getString(R.string.ERROR_INTERNET_CONNECTION))
            return
        }

        val call: Call<Boolean> = if (comlete) {
            mRemote.complete(id)
        } else {
            mRemote.undo(id)
        }
    ...
    private fun callRemote(call: Call<Boolean>, listener: APIListener<Boolean>) {
